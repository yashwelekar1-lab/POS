...
export function Analytics() {
  ...
  const downloadInvoice = async () => {
    ...
    try {
      // Use the browser's native print dialog so the invoice can be saved as PDF
      // without requiring a PDF library at runtime.
      window.print();
    } catch (err) {
      console.error(err);
      setError("Unable to open the invoice print dialog.");
    } finally {
